    ...
    <div className="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:w-72 lg:flex-col">
      <div className="flex grow flex-col gap-y-5 overflow-y-auto border-r border-glow-100 bg-white px-6 pb-4">
        <div className="flex h-16 shrink-0 items-center gap-2">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-glow-400 to-lavender-500">
            <Sparkles className="h-4 w-4 text-white" />
          </div>
          <span className="font-bold text-gray-900">
            GLOWUP<span className="text-glow-500">.AI</span>
          </span>
        </div>
        <div className="rounded-2xl bg-gradient-to-br from-glow-500 to-lavender-500 p-4 text-white">
          ...
          <p className="text-sm opacity-90">
            {profile?.subscription_tier === 'free' ? '3 scans/month' : 'Unlimited'}
          </p>
        </div>
          ...
          {nav.map((item) => (
            <Link
              ...
              className={cn(
                ...
                pathname === item.href ? 'bg-glow-50 text-glow-700' : 'text-gray-600 hover:bg-gray-50'
              )}
            >
        ...
        <button
          ...
          className="flex items-center gap-x-3 rounded-xl px-3 py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-50"
        >
